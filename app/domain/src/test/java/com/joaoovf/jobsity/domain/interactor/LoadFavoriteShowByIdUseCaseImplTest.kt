...
class LoadFavoriteShowByIdUseCaseImplTest {
	...
	fun `Given show  When find by show id Then return Show`() = runTest {
		...
		val loadFavoriteShowByIdUseCaseImpl = LoadFavoriteShowByIdUseCaseImpl(fakeRepository)

		val result = loadFavoriteShowByIdUseCaseImpl(show.id)
	...
	fun `Given show When find by with another id Then return null`() = runTest {
		...
		val result = loadFavoriteShowByIdUseCaseImpl(showId = 2)
